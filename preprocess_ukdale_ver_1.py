...
def read_dataframe(house_nr, channel_type, channel_nr = "", appliance = "appliance"):
    ...
    channel_type: Either "labels", "channel" or "mains" should be chosen
    ...
    if channel_type == "labels":
        ...
        df = pd.read_table(file_name, sep = "\s+", names = ['unix', appliance]) 
    elif channel_type == "mains":
        file_name = f"ukdale/house_{house_nr}/mains.dat"
        df = pd.read_table(file_name, sep = "\s+", names = ['unix', 'mains', 'a', 'b'])
    return df
...
def determine_appl_lst(house_nr):
    ...
    return [df_labels.appliance.tolist()[i] + "_" + str(df_labels.unix.tolist()[i]) for i in range(df_labels.shape[0])]
